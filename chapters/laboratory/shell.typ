...
- `bash`: Esegue i comandi mv generati.

=== Esercizio 7 (fatto all'esame)
Scrivere un comando a riga di comando che permetta di trovare il file .h in cui la parola "include" compare più volte, restituendo come output il numero di occorrenze e il nome del file, senza spazi iniziali.
```bash
find usr/include -type f -name '*.h' -exec grep -o -H "include" {} \; | cut -d ':' -f 1 | uniq -c | sort -nr | head -1 | sed 's/^\ *//g'
```

Questo comando esegue una serie di operazioni concatenate per trovare il file di intestazione (`.h`) nella directory `/usr/include/` che contiene più occorrenze della parola `"include"`. Ecco il commento dettagliato per ogni blocco:  

- `find usr/include -type f -name '*.h'`:  
   - Cerca tutti i file regolari (`-type f`) con estensione `.h` all'interno della directory `usr/include/`.  
   - `find` restituirà un elenco dei percorsi completi dei file trovati.  

- `-exec grep -o -H "include" {} \;`  
   - Per ogni file trovato, esegue `grep` per cercare la stringa `"include"`.  
   - `-o` fa sì che `grep` stampi solo le occorrenze trovate, invece dell'intera riga.  
   - `-H` stampa il nome del file prima di ogni risultato, nel formato `<file>:include`.  

- `cut -d ':' -f 1`  
   - Estrae solo il nome del file rimuovendo la parte `"include"` dal risultato di `grep`.  
   - Il delimitatore `:` viene usato per separare il nome del file dal testo trovato.  

- `uniq -c`: Conta il numero di volte in cui ogni file appare nell'output, ovvero il numero di occorrenze della parola `"include"` per file.  

- `sort -nr`  
   - Ordina i risultati in ordine numerico decrescente (`-n` per numerico, `-r` per invertire l'ordine).  
   - Questo assicura che il file con il maggior numero di occorrenze venga mostrato per primo.  

- `| head -1`: Seleziona solo la prima riga dell'output, ovvero il file con il maggior numero di occorrenze di `"include"`.  

- `sed 's/^\ *//g'`: Rimuove eventuali spazi iniziali nell'output finale.